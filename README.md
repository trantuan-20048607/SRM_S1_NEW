...
1. <font color=#FFFF00>一般</font> 录制视频时有严重丢帧；（已修复延迟，操作不受影响）
2. <font color=#FF0000>严重</font> 远距离自瞄对齐时会产生未知错误导致闪退。

### 注意事项 ###

1. <font color=#00FF00>影响较小</font> 由于控制进程和界面进程先后启动，程序开始运行时会短暂延迟；
2. <font color=#FF0000>可能导致异常</font> 由于 KALMAN 滤波器的特性，切换至 KALMAN 滤波器后目标位置会短暂漂移；
3. <font color=#FFFF00>影响中等</font> 由于操作传输机制问题，操作在实机上会表现卡顿。
