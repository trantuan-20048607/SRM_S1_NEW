...
class CfgNode(dict):
    ...
    def __init__(self, init_dict=None, key_list=None, new_allowed=False):
        """
        Args:
            init_dict (dict): the possibly-nested dictionary to initialize the CfgNode.
    ...
    def set_new_allowed(self, is_new_allowed):
        """
        Set this config (and recursively its sub configs) to allow merging
    ...
    @classmethod
    def decode_cfg_value(cls, value):
        """
...
def _check_and_coerce_cfg_value_type(replacement, original, _, full_key):
    ...
    if (isinstance(None, replacement_type) and original_type in _VALID_TYPES) or (
            isinstance(None, original_type) and replacement_type in _VALID_TYPES
    ):
        return replacement
